...
export class BooksComponent implements OnInit {
  ...
  constructor(
    ...
    private favoritesService: BuyRentService
  ) {
  ...
  getBooksList() {
    ...
    this.backeEndService
      .searchBookList(this.searchTerm)
      .subscribe((booksList) => {
  ...
  likedIcon(isliked: boolean, book: Book) {
    if (isliked) return this.favoritesList.includes(book);
    else {
  ...
  buyIcon(isliked: boolean, book: Book) {
    if (isliked) return this.boughtList.includes(book);
    else {
